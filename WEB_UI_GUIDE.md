...
1. 默认情况下，语音输出功能是开启的（蓝色扬声器图标）
2. 秒神酱的回复会自动使用带有情感的可爱语音读出
3. 语音包含自然的停顿和语调变化，使其听起来更加自然、有表现力
4. 中文和英文内容会自动使用相应的语音进行读出
5. 您可以点击扬声器图标关闭语音输出功能（图标变为灰色）
6. 再次点击可以重新开启语音输出
...
- 语音输出包含情感表达和自然停顿，使其听起来更加自然、有表现力
- 语音输出的音调和速度已经精心调整，使其听起来更加可爱、有活力
- 自动识别文本中的表情符号和语气词，增强情感表达
...
如果中文语音输出不正常，请尝试以下解决方法：

1. 在浏览器设置中添加中文语言包
2. 尝试使用Chrome浏览器，它通常具有最好的中文语音支持
3. 刷新页面后再次尝试，有时语音需要一段时间才能加载完成
4. 检查浏览器控制台（F12）中是否有相关错误信息
